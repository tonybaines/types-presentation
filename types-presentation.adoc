...
Optional<MeetingRequest> bookMeeting(Instant startTime, Duration duration) {...}


MeetingRequest meetingRequest =
    nextFreeMeetingSlot(criteria)
        .filter(timeSlot -> myDiary.isFree(timeSlot))
        .map(timeSlot -> timeSlot.startTime)
        .flatMap(startTime -> bookMeeting(startTime, minutes(15)))
        .orElseThrow(new IllegalStateException("No meeting slot available"));
----
...
* This Haskell code expresses the concept that billing information can be
